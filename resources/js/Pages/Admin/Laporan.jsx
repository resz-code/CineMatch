        ...
        <AdminLayout>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
                {[
                    ...
                    <div key={s.label} className="bg-[#1a1a1a] border border-zinc-800 rounded-xl p-4 shadow-sm">
                    ...
                    <div className="flex flex-col gap-3">
                        {topFilm.map((film, index) => (
                            <div key={film.judul} className="flex items-center gap-3">
                                <div className="w-6 h-6 rounded-full bg-purple-950/50 border border-purple-800/30 text-purple-400 text-xs flex items-center justify-center font-semibold flex-shrink-0">
                                    {index + 1}
                                </div>
                                <div className="flex-1">
                                    <div className="text-sm font-medium text-white">{film.judul}</div>
                                    <div className="w-full bg-[#141414] rounded-full h-1 mt-1 border border-zinc-800/50">
                                        <div
                                ...
                                <div className="text-xs text-yellow-500 font-medium">★ {film.rating}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="bg-[#1a1a1a] border border-zinc-800 rounded-xl p-5 shadow-sm">
                ...
                <table className="w-full text-sm text-zinc-300">
                    <thead className="border-b border-zinc-800">
                        <tr>
                            <th className="text-left text-xs text-zinc-400 font-medium pb-3 uppercase tracking-wider">Hari</th>
                            <th className="text-left text-xs text-zinc-400 font-medium pb-3 uppercase tracking-wider">User aktif</th>
                            <th className="text-left text-xs text-zinc-400 font-medium pb-3 uppercase tracking-wider">Rating masuk</th>
                            <th className="text-left text-xs text-zinc-400 font-medium pb-3 uppercase tracking-wider">Rata-rata rating</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-800/40">
                        {aktivitasHarian.map((a) => (
                            <tr key={a.hari} className="hover:bg-[#222222]/20 transition-colors">
                                <td className="py-2.5 font-semibold text-white">{a.hari}</td>
                                <td className="py-2.5 font-medium text-zinc-300">{a.user}</td>
                                <td className="py-2.5 font-medium text-zinc-300">{a.rating}</td>
                                <td className="py-2.5 text-yellow-500 font-medium">★ {(a.rating / a.user).toFixed(1)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
