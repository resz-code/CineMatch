...
export default function Profil() {
    ...
    const [user, setUser] = useState(
        {name: 'Memuat...', email: 'memuat@cinematch.com'}
    );
    ...
    const submitGenres = async () => {
        ...
        try {
            ...
            setGenreStatus(
                {type: 'success', message: 'Preferensi genre berhasil disimpan!'}
            );
        } catch (error) {
    ...
    if (isLoading) {
        return <div
            className="min-h-screen bg-[#141414] flex items-center justify-center text-zinc-500">Memuat data profil...</div>;
    }
                    ...
                    <div className="col-span-1">
                        <div className="bg-[#1a1a1a] border border-zinc-800 rounded-xl p-5">
                            ...
                            <div className="text-center mb-5">
                                <div
                                    className="w-16 h-16 rounded-full bg-purple-950/40 border border-purple-900/40 flex items-center justify-center text-2xl font-medium text-purple-400 mx-auto mb-3">
                                    {
                                        user
                                            .name
                                            .charAt(0)
                            ...
                            <div className="grid grid-cols-2 gap-2 mb-5">
                                <div
                                    className="bg-[#222222] border border-zinc-800/60 rounded-lg p-2 text-center">
                                    <div className="font-medium text-base text-white">0</div>
                            ...
                            <div className="border-t border-zinc-800/60 pt-4">
                                {
                                    menus.map((menu) => (
                                        <button
                                            ...
                                            activeMenu === menu.id
                                                ? 'bg-purple-600 border border-purple-500 text-white shadow-sm shadow-purple-500/' +
                                                        '10'
                                ...
                                activeMenu === 'edit' && (
                                    <div>
...

                                        {
                                            editStatus.message && (
                                                <div
                                                    className={`mb-4 px-4 py-2 rounded-lg text-xs font-medium ${editStatus.type === 'success'
                                        ...
                                        <form onSubmit={submitEdit}>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                                                <div>
                                                    <label
                                                        className="block text-xs text-zinc-400 font-medium mb-2 uppercase tracking-wider">Nama lengkap</label>
                                                    <input
                                                        type="text"
                                                        value={data.name}
                                                        onChange={(e) => setData({
                                                ...
                                                <div>
                                                    <label
                                                        className="block text-xs text-zinc-400 font-medium mb-2 uppercase tracking-wider">Email</label>
                                                    <input
                                                        type="email"
                                                        value={data.email}
                                                        onChange={(e) => setData({
                                ...
                                activeMenu === 'genre' && (
                                    <div>
...

                                        {
                                            genreStatus.message && (
                                                <div
                                                    className={`mb-4 px-4 py-2 rounded-lg text-xs font-medium ${genreStatus.type === 'success'
                                                        ? 'bg-emerald-950/30 text-emerald-400 border border-emerald-900/30'
                                        ...
                                        <div className="flex items-center gap-2 mb-5">
                                            ...
                                            <span className="text-xs font-medium text-purple-400">{selectedGenres.length}
                                                genre</span>
                                        </div>
                                        <button
                                ...
                                activeMenu === 'password' && (
                                    <div>
...

                                        {
                                            passStatus.message && (
                                                <div
                                                    className={`mb-4 max-w-md px-4 py-2 rounded-lg text-xs font-medium ${passStatus.type === 'success'
                                                        ? 'bg-emerald-950/30 text-emerald-400 border border-emerald-900/30'
                                                        : 'bg-red-950/30 text-red-400 border border-red-900/30'}`}>
                                                    {passStatus.message}
                                                </div>
                                        ...
                                        <form onSubmit={submitPassword}>
                                            <div className="mb-4 max-w-md">
                                                <label
                                                    className="block text-xs text-zinc-400 font-medium mb-2 uppercase tracking-wider">Password lama</label>
                                                <input
                                                    type="password"
                                            ...
                                            <div className="mb-4 max-w-md">
                                                <label
                                                    className="block text-xs text-zinc-400 font-medium mb-2 uppercase tracking-wider">Password baru</label>
                                                <input
                                                    type="password"
                                            ...
                                            <div className="mb-6 max-w-md">
                                                <label
                                                    className="block text-xs text-zinc-400 font-medium mb-2 uppercase tracking-wider">Konfirmasi password baru</label>
                                                <input
                                                    type="password"
